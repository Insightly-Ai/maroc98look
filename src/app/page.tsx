...
const C = { green: "#006233", red: "#C1272D", gold: "#A8862C", goldLight: "#D4A843", cream: "#F5F0E8", dark: "#1A1A1A", gray: "#6B7280", lightGray: "#F9FAFB", border: "#E5E7EB" };
...
function PaniniCard({ name, rotate = 0, imgSrc }: { name: string; rotate?: number; imgSrc?: string }) {
  ...
  return (
    <div style={{ transform: `rotate(${rotate}deg)`, transition: "transform 0.3s ease" }}>
      <div style={{ width: "130px", background: "#E8DFC8", border: "3px solid #C1272D", borderRadius: "4px", boxShadow: "0 4px 16px rgba(0,0,0,0.18)", overflow: "hidden", fontFamily: "Arial, sans-serif" }}>
...
function ChampionCard({ rotate = 0, imgSrc }: { rotate?: number; imgSrc?: string }) {
  if (imgSrc) {
    return (
      <div style={{ transform: `rotate(${rotate}deg)`, transition: "transform 0.3s ease" }}>
        <img src={imgSrc} alt="Champion photo" style={{ width: "130px", borderRadius: "6px", boxShadow: "0 4px 16px rgba(0,0,0,0.2)", display: "block" }} />
      </div>
    );
  }
  return (
    <div style={{ transform: `rotate(${rotate}deg)`, transition: "transform 0.3s ease" }}>
      <div style={{ width: "130px", height: "160px", borderRadius: "6px", overflow: "hidden", boxShadow: "0 4px 16px rgba(0,0,0,0.22)", background: "linear-gradient(160deg, #8B0000 0%, #C1272D 30%, #1a0005 100%)" }} />
    </div>
  );
}
  ...
  const handleFile = useCallback((file: File, fromFrontCamera = false) => {
    ...
    reader.onload = (e) => {
      ...
      img.onload = () => {
        ...
        if (fromFrontCamera) { ctx.translate(width, 0); ctx.scale(-1, 1); }
  ...
  return (
    <div style={{ minHeight: "100vh", background: "#fff", fontFamily: "'Helvetica Neue', Arial, sans-serif", color: C.dark }}>
      <style>{`
        ...
        .product-card:hover { border-color: ${C.gold} !important; transform: translateY(-2px); box-shadow: 0 4px 20px rgba(0,0,0,0.08) !important; }
      ...
      <div style={{ maxWidth: "1200px", margin: "0 auto", display: "flex", alignItems: "flex-start", gap: "24px", padding: "32px 24px 80px" }}>
        ...
        <div className="side-panel" style={{ width: "clamp(100px, 12vw, 160px)", flexShrink: 0, flexDirection: "column", alignItems: "center", gap: "20px", paddingTop: "16px" }}>
          <PaniniCard name="HASSNA" rotate={-4} imgSrc="/examples/panini-hasna.jpeg" />
          <ChampionCard rotate={3} imgSrc="/examples/champion-1.jpeg" />
          <PaniniCard name="YOUNES" rotate={-2} imgSrc="/examples/panini-younes.jpeg" />
        </div>
        ...
        <div style={{ flex: 1, minWidth: 0, maxWidth: "640px", margin: "0 auto" }}>
          ...
          {!image && (
            <div className="mobile-examples" style={{ gap: "10px", marginBottom: "24px", justifyContent: "center" }}>
              <PaniniCard name="HASSNA" rotate={-3} imgSrc="/examples/panini-hasna.jpeg" />
              <ChampionCard rotate={2} imgSrc="/examples/champion-1.jpeg" />
              <PaniniCard name="YOUNES" rotate={3} imgSrc="/examples/panini-younes.jpeg" />
            </div>
          )}
          ...
          <div style={{ background: "#fff", border: `1px solid ${C.border}`, borderRadius: "16px", padding: "24px", marginBottom: "16px", boxShadow: "0 2px 12px rgba(0,0,0,0.06)" }}>
            ...
            {image ? (
              <div
                ...
                onDrop={handleDrop}
                style={{ border: `2px dashed ${dragOver ? C.gold : C.border}`, borderRadius: "12px", padding: "16px", textAlign: "center", transition: "all 0.2s" }}>
              ...
              <div
                ...
                onDragLeave={() => setDragOver(false)}
                onDrop={handleDrop}
                style={{ border: `2px dashed ${dragOver ? C.gold : C.border}`, borderRadius: "12px", padding: "24px 16px", textAlign: "center", transition: "all 0.2s", background: dragOver ? C.cream : C.lightGray }}>
              ...
              <div style={{ display: "flex", gap: "12px", marginBottom: productType === "panini" ? "16px" : "0" }}>
                {([
                  ...
                  <div key={type} className="product-card" onClick={() => setProductType(type)} style={{
                    ...
                    background: productType === type ? "#FFF8F8" : "#fff",
          ...
          {error && (
            <div style={{ background: "#FFF0F0", border: `1px solid rgba(193,39,45,0.2)`, borderRadius: "8px", padding: "12px 16px", color: C.red, fontSize: "14px", marginBottom: "16px", textAlign: "center", fontWeight: 600 }}>
              {error}
            </div>
          )}
            ...
            <div className="result-card" style={{ background: "#fff", border: `1px solid ${C.border}`, borderRadius: "16px", padding: "24px", boxShadow: "0 4px 24px rgba(0,0,0,0.08)" }}>
              ...
              <div style={{ marginTop: "16px", display: "flex", gap: "10px", justifyContent: "center", flexWrap: "wrap" as const }}>
                <button onClick={async () => {
                  const shareText = "Check out my Legend World Cup Morocco photo. Create yours at: https://www.worldcuplegend.nl/morocco";
                  try {
                ...
                <button onClick={async () => {
                  try {
                    ...
                    else { const a = document.createElement("a"); a.href = resultUrl; a.download = "morocco-2026-legend.jpg"; a.click(); }
                  } catch { const a = document.createElement("a"); a.href = resultUrl; a.download = "morocco-2026-legend.jpg"; a.click(); }
                }} style={{
          ...
          <div style={{ marginTop: "48px", paddingTop: "24px", borderTop: `1px solid ${C.border}`, textAlign: "center" }}>
            <div style={{ fontSize: "12px", color: C.gray, lineHeight: 1.8 }}>
              <div style={{ fontWeight: 700, color: C.dark, marginBottom: "4px" }}>worldcuplegend.nl</div>
              <div>Contact: <a href="mailto:info@worldcuplegend.nl" style={{ color: C.red, textDecoration: "none" }}>info@worldcuplegend.nl</a></div>
              <div style={{ marginTop: "8px", opacity: 0.7 }}>Payments secured by Stripe · Images generated by AI · Photo data never stored</div>
              <div style={{ marginTop: "4px", opacity: 0.7 }}>© 2026 worldcuplegend.nl · Morocco 2026 FIFA World Cup Fan Project</div>
            </div>
          </div>
        </div>
        ...
        <div className="side-panel" style={{ width: "clamp(100px, 12vw, 160px)", flexShrink: 0, flexDirection: "column", alignItems: "center", gap: "20px", paddingTop: "16px" }}>
          ...
          <PaniniCard name="HICHAM" rotate={4} imgSrc="/examples/panini-hicham.jpeg" />
          <ChampionCard rotate={-2} imgSrc="/examples/champion-3.jpg" />
        </div>
      </div>
